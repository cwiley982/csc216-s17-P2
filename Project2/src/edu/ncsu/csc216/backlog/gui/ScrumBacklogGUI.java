...
public class ScrumBacklogGUI extends JFrame implements ActionListener {
	...
	/**
	 * Constructs a {@link ScrumBacklogGUI} object that will contain a
	 * {@link JMenuBar} and a {@link JPanel} that will hold different possible
	 * views of the data in the {@link ScrumBacklogModel}.
	 */
	 ...
	 * that shows the list of task items.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class TaskItemListPanel extends JPanel implements ActionListener {
		...
		/**
		 * Text field for a user to enter an creator name to filter TaskItem
		 * list
		 */
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == btnAddTaskItem) {
				// If the add button is pressed switch to the
				// createTaskItemPanel
		...
		/**
		 * Updates the TaskItem table with items that may be filtered by owner
		 * or creator.
		 * 
		 * @param owner
		 *            user id to filter by owner
		 * @param creator
		 *            user id to filter by creator
		 */
		...
		/**
		 * {@link TaskItemTableModel} is the object underlying the
		 * {@link JTable} object that displays the list of {@link TaskItem}s to
		 * the user.
		 * 
		 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
		 */
		private class TaskItemTableModel extends AbstractTableModel {
			...
			/**
			 * Constructs the {@link TaskItemTableModel} by requesting the
			 * latest information from the {@link ScrumBacklogModel}.
			 */
			...
			/**
			 * Updates the given model with {@link TaskItem} information from
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with a backloged TaskItem.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class BacklogPanel extends JPanel implements ActionListener {
		...
		private static final long serialVersionUID = 1L;
		/**
		 * {@link TaskItemInfoPanel} that presents the {@link TaskItem}'s
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with an owned task.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class OwnedPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() != btnCancel && note.equals("")) {
				...
				try {
					Command c = new Command(Command.CommandValue.PROCESS,
							ScrumBacklogModel.getInstance().getTaskItemById(taskItemId).getOwner(), note);
					ScrumBacklogModel.getInstance().executeCommand(taskItemId, c);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(ScrumBacklogGUI.this, "Invalid command");
					reset = false;
				...
				// Try a command. If problem, go back to task item list.
				try {
					Command c = new Command(Command.CommandValue.REJECT,
							ScrumBacklogModel.getInstance().getTaskItemById(taskItemId).getOwner(), note);
					ScrumBacklogModel.getInstance().executeCommand(taskItemId, c);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(ScrumBacklogGUI.this, "Invalid command");
				...
				// Try a command. If problem, go back to task item list.
				try {
					Command c = new Command(Command.CommandValue.BACKLOG,
							ScrumBacklogModel.getInstance().getTaskItemById(taskItemId).getOwner(), note);
					ScrumBacklogModel.getInstance().executeCommand(taskItemId, c);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(ScrumBacklogGUI.this, "Invalid command");
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with a processing task.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class ProcessingPanel extends JPanel implements ActionListener {
		...
		/**
		 * Constructs a JFrame for editing a {@link TaskItem} in the
		 * ProcessingState.
		 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() != btnCancel && note.equals("")) {
				...
				// Try command. If problem, go to task list.
				try {
					Command c = new Command(Command.CommandValue.VERIFY,
							ScrumBacklogModel.getInstance().getTaskItemById(taskItemId).getOwner(), note);
					ScrumBacklogModel.getInstance().executeCommand(taskItemId, c);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(ScrumBacklogGUI.this, "Invalid command");
				...
				try {
					Command c = new Command(Command.CommandValue.COMPLETE,
							ScrumBacklogModel.getInstance().getTaskItemById(taskItemId).getOwner(), note);
					ScrumBacklogModel.getInstance().executeCommand(taskItemId, c);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(ScrumBacklogGUI.this, "Invalid command");
				...
				// Otherwise, try a Command. If command fails, go back to task
				// list
				try {
					Command c = new Command(Command.CommandValue.BACKLOG,
							ScrumBacklogModel.getInstance().getTaskItemById(taskItemId).getOwner(), note);
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with a verifying task item.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class VerifyingPanel extends JPanel implements ActionListener {
		...
		/**
		 * Constructs a JPanel for editing a {@link TaskItem} in the
		 * VerifyingState.
		 */
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with a done task item.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class DonePanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() != btnCancel && note.equals("")) {
				...
				try {
					Command c = new Command(CommandValue.PROCESS,
							ScrumBacklogModel.getInstance().getTaskItemById(taskItemId).getOwner(), note);
					ScrumBacklogModel.getInstance().executeCommand(taskItemId, c);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(ScrumBacklogGUI.this, "Invalid command");
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that interacts with a rejected task item.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that shows information about the task item.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class TaskItemInfoPanel extends JPanel {
		...
		public TaskItemInfoPanel() {
			...
			scrollNotes = new JScrollPane(tableNotes, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
		 ...
		 * 
		 * @param taskItemId
		 *            the id for the task item to display information about.
		 */
		public void setTaskItemInfo(int taskItemId) {
			// Get the task from the model
			TaskItem t = ScrumBacklogModel.getInstance().getTaskItemById(taskItemId);
			if (t == null) {
				// If the task item doesn't exist for the given id, show an
				// error message
		...
		/**
		 * {@link NotesTableModel} is the object underlying the {@link JTable}
		 * object that displays the list of Notes to the user.
		 * 
		...
		private class NotesTableModel extends AbstractTableModel {
			...
			/**
			 * Updates the given model with {@link NoteItem} information from
			 * the {@link TaskItem}.
			 */
	...
	/**
	 * Inner class that creates the look and behavior for the {@link JPanel}
	 * that allows for creation of a new task item.
	 * 
	 * @author Dr. Sarah Heckman (heckman@csc.ncsu.edu)
	 */
	private class CreateTaskItemPanel extends JPanel implements ActionListener {
		...
		/**
		 * Creates the {@link JPanel} for adding new task item to the backlog.
		 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAdd) {
				...
				if (idx < 0 || idx >= types.length) {
					...
					switch (idx) {
					case 0:
						type = TaskItem.Type.FEATURE;
						break;
					case 1:
						type = TaskItem.Type.BUG;
						break;
					case 2:
						type = TaskItem.Type.TECHNICAL_WORK;
						break;
					case 3:
						type = TaskItem.Type.KNOWLEDGE_ACQUISITION;
						break;
					default:
						type = null;
					}
